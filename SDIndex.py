...
# Load Yolo Model
    ...
    net = cv2.dnn.readNet("C:/Users/lenovo/OneDrive/Desktop/care_from_covid/yolov3.weights", "C:/Users/lenovo/OneDrive/Desktop/care_from_covid/yolov3.cfg")
    classes = ["person"]

    with open("C:/Users/lenovo/OneDrive/Desktop/care_from_covid/coco.names", "r") as f:
    ...
    while(cap.isOpened()):
        ...
        if len(indexes)>0:        
            ...
            x=(safe_p/total_p)
            e=e+x
            high=high+(high_risk_p/total_p)
            lower=lower+(low_risk_p/total_p)
            totalno=totalno+1

            Totalpeople.append(total_p)
            Redline.append(high_risk_p)
            Greenline.append(safe_p)
            Orangeline.append(low_risk_p)
            totalframe.append(totalno)

            if totalno%10==0:
                check50+=1
                check51.append(totalno)
                totalaver=sum(Totalpeople)/10
                aver=sum(Redline)/10
                aver1=sum(Greenline)/10
                aver2=sum(Orangeline)/10
                TotalAverageline.append(totalaver)
                AverageRedline.append(aver)
                AverageGreenline.append(aver1)
                AverageOrangeline.append(aver2)
                Totalpeople.clear()
                Redline.clear()
                Greenline.clear()
                Orangeline.clear()
        ...
        output = cv2.VideoWriter('output1.mp4',fourcc, 20.0, (img.shape[1], img.shape[0]))
    ...
    e=(e/totalno)*100
    high=(high/totalno)*100
    lower=(lower/totalno)*100
    high=round(high,2)
    lower=round(lower,2)
    e=round(e,2)
    return e, high, lower, AverageGreenline, AverageRedline, TotalAverageline, check51
